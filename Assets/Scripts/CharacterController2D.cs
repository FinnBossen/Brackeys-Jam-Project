...
public class CharacterController2D : MonoBehaviour
{
	...
	public GameObject crouchIdleObject; 
	public GameObject idleObject;
	public GameObject crouchMovingObject; 
	...
	public void Move(float move, bool crouch, bool jump, bool isMoving)
	{
		...
		if ((m_Grounded || m_AirControl )&& !airCollision)
		{

			switchIdle(crouch, isMoving);
			// If crouching
			if (crouch)
			{
				if (!m_wasCrouching)
	...
	public void switchIdle(bool crouch, bool isMoving)
	{
		if (m_Grounded)
		{
			...
			if (crouch)
			{
				...
				if (isMoving)
				{
					crouchIdleObject.SetActive(false);
					crouchMovingObject.SetActive(true);
				}
				else
				{
					crouchIdleObject.SetActive(true);
					crouchMovingObject.SetActive(false);
				}
			}
			else
			{
	...
	private void OnTriggerEnter2D(Collider2D collision)
	{
		...
		{
			originalPos = new Vector3(collision.transform.position.x, collision.transform.position.y, -7.87f);
		}
	}
}